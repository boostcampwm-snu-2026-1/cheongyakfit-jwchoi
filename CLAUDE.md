...
청약핏: 민영 아파트 청약의 **[A] 자격 판정** + **[B] 당첨 시 자금 계획(자기자본·대출)** 분석 웹서비스. (제품 설명은 [README](README.md), 범위는 [scope](docs/scope.md))

## 비협상 규칙 (어기지 말 것)

1. **CORE는 순수.** `lib/core/`(matching·finance·rules·types)는 Next.js·Supabase·OpenAI를 **import 금지**. 부수효과는 `lib/server/`에만.
2. **매칭·자금 엔진에 LLM 금지.** 판정·계산은 결정론적 순수 함수. LLM은 파싱(`lib/server/parsing/`)에서만.
3. **기준표 값은 코드에 직접 쓰지 말 것.** 소득/가점/예치금/LTV/DSR 값의 SSoT는 [docs/domain/](docs/domain/README.md). 코드는 그걸 옮기고 `// SSoT: docs/domain/finance.md §...` 역참조 주석을 단다. 문서 먼저 고치고 **같은 PR에서 코드 동기화**.
4. **신뢰할 수 없는 입력은 zod로 검증.** 폼 입력·LLM 파싱 결과는 `lib/schemas/`의 zod를 통과시킨 뒤 사용.
5. **RLS 우회 금지.** 프로필=민감정보(소득·기존대출). DB 접근은 **supabase-js**로(=Prisma 금지). 본인 데이터만 접근.
...
- **파일 위치**: 화면→`app/`, 부수효과→`lib/server/{parsing,db,storage,auth}`, 순수 로직→`lib/core/{matching,finance,rules}`, zod→`lib/schemas/`, DB 정의→`supabase/`.

## 작업 플레이북

- **Phase 구현에 착수** → 코드 전에 `docs/plans/phaseN-*.md`에 계획을 먼저 쓴다([scope](docs/scope.md) DoD 참조, 파일 순서·시그니처·테스트 목록). 검토 후 구현. 플랜은 Phase별 하나씩.
- **청약 룰/기준표를 추가·수정** → 먼저 [docs/domain/](docs/domain/README.md)(eligibility 또는 finance)를 고치고 → `lib/core/rules`·엔진에 반영(역참조 주석).
